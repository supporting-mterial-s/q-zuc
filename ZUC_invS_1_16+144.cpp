...
void ZUC_invS_1_T_DEPTH3(int *in, int *S)
{
...
tmp_5 = tmp_5 ^ in[0];
in[0] = in[0] ^ in[5];//  y[3
tmp_4 = tmp_4 ^ in[5];//  y[2
tmp_6 = tmp_6 ^ in[1];
tmp_6 = tmp_6 ^ in[0];//  y[1
...
in[1] = in[1] ^ in[3]; // y[5
tmp_10 = tmp_10 ^ in[5];
tmp_10 = tmp_10 ^ in[2];  //y[7
in[0] = in[0] ^ in[5];  //y[3
tmp_12 = tmp_12 ^ in[7]; // y[11
in[6] = in[6] ^ in[1]; // y[9
in[7] = in[7] ^ in[0]; // y[8
tmp_11 = tmp_11 ^ in[6]; // y[10
in[3] = in[3] ^ in[0]; // y[4
tmp_9 = tmp_9 ^ in[0];// y[6
...
int main()
{
	for(int i = 0; i < 256; i++)
	{
		...
		for(int j = 0; j < 8; j++)
		{
			in[j] = ((sbox[i]>>(7 - j)) & 0x01);
		}

   ZUC_invS_1_T_DEPTH3(in, out);

   int sum = out[2] * 128 + out[1] * 64 + out[6] * 32 + out[7] * 16 + out[4] * 8 + out[5] * 4 + out[3] * 2 + out[0];
     ...
     if ((i != 0) && ((i % 16 == 0)))
	{
     ...
     cout<<(sum^i)<<" ";//sum = i = inverse of S-box(i);
	}
     cout << endl;
	return 0;
}
...
Totally,
 Measure : 78;
 1-qubit : 8 + 78*(3+4) = 554;
 CNOT    : 699 + 78*(8+1) = 1401;
 T       : 78 * 4 = 312
